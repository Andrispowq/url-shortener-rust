...
pub trait LinkServiceTrait {
    ...
    fn create_link(&mut self, dto: CreateLinkDto) -> Result<LinkDto, LinkError>;
    fn visit_link(&mut self, code: String) -> Result<LinkDto, LinkError>;
}
...
impl LinkServiceTrait for LinkService {
    ...
    fn visit_link(&mut self, code: String) -> Result<LinkDto, LinkError> {
        let link = self
            ...
            .ok_or_else(|| LinkError::LinkNotFoundByCode { code })?;
...
mod tests {
    ...
    fn visit_unknown_code_returns_error() {
        ...
        assert_eq!(error, LinkError::LinkNotFoundByCode { code: "missing".to_string() });
    }
}
